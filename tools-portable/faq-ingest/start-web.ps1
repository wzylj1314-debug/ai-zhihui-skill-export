...
$env:ZHIHUI_FAQ_TOOL_DIR = $PSScriptRoot
$env:ZHIHUI_FAQ_PORT = "$Port"
...
Write-Host "Starting FAQ workbench on $HostAddress`:$Port"
if ($Lan) {
  ...
  foreach ($ip in $ips) {
    ...
    Write-Host "Board:   http://$ip`:$Port/board"
  }
  Write-Host "Warning: LAN mode exposes screenshot drafts to devices on the same network."
} else {
  Write-Host "Local URL: http://127.0.0.1:$Port/"
  Write-Host "Board:     http://127.0.0.1:$Port/board"
}
